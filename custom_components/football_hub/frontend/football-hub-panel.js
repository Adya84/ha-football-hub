...
const FULL_COMPETITION_CATALOGUE = {
  ...
  Portugal: ["Primeira Liga", "TaÃ§a de Portugal", "Portuguese League Cup", "Portuguese Super Cup"],
  Belgium: ["Jupiler Pro League", "Belgian Cup", "Belgian Super Cup"],
  TÃ¼rkiye: ["SÃ¼per Lig", "Turkish Cup", "Turkish Super Cup"],
...
class FootballHubPanel extends HTMLElement {
  ...
  _countryFlag(country, className = "supporter-flag-image") {
    ...
    if (!code) return `<span class="supporter-flag-fallback">ðŸ³ï¸</span>`;
  ...
  _lmsEmailServices() {
    ...
    const entities = Object.entries(this._hass?.states || {})
      ...
      .map(([entityId, state]) => ({
        value: entityId,
        label: `${state?.attributes?.friendly_name || entityId.slice(7).replaceAll("_", " ")} Â· notify entity`,
        modern: true,
      }));
    const legacy = Object.entries(services)
      ...
      .map(([service, details]) => ({
        ...
        label: `${details?.name || details?.description || service.replaceAll("_", " ")} Â· legacy`,
        modern: false,
      }));
    return [{ value: "__auto__", label: "Automatically match each player email Â· recommended", modern: true }, ...entities, ...legacy.filter((item) => !entities.some((entity) => entity.value === item.value))]
  ...
  _buyBackLmsPlayer(playerId) {
    ...
    if (!window.confirm(`Buy ${player.name} back into the competition for Â£${fee.toFixed(2)}? Their first team will remain used.`)) return;
  ...
  _rolloverLmsCompetition() {
    ...
    if (!window.confirm(`Rollover Â£${currentPrize.toFixed(2)} into a new competition? Every player will need to pay and choose again.`)) return;
  ...
  _doublePickEmailSummary(includeResults = false) {
    ...
    const lines = (game.players || []).map((player) => {
      ...
      const pickText = Array.from({ length: Number(game.pickCount || 2) }, (_, slot) => {
        ...
        return `${team}${includeResults && result ? ` â€” ${result.outcome}, ${result.score}, ${result.points} points` : ""}`;
      }).join(" + ");
      return `${player.name}: ${pickText}`;
    });
    const payer = game.players.find((player) => player.id === round.payerId)?.name || "Not recorded";
    const money = includeResults ? `\nStake: Â£${Number(round.stake || 0).toFixed(2)}\nReturn: Â£${Number(round.returnAmount || 0).toFixed(2)}\nProfit/loss: Â£${(Number(round.returnAmount || 0) - Number(round.stake || 0)).toFixed(2)}` : "";
  ...
  async _emailDoublePickRound(kind) {
    ...
    for (const player of game.players || []) {
      ...
      let title, message;
      if (kind === "reminder") {
        title = `${game.name} Â· Round ${game.round} picks`;
        ...
        title = `${game.name} Â· Round ${game.round} selections`;
        message = `The selections are confirmed:\n\n${this._doublePickEmailSummary(false)}`;
      } else {
        title = `${game.name} Â· Round ${game.round} results`;
  ...
  _doublePickLeaguePage() {
    ...
    const pickOptions = (selected = "") => `<option value="">Choose a team</option>${optionGroups.filter((group) => group.teams.length).map((group) => `<optgroup label="${this._escape(`â”€â”€ ${String(group.country || "").toUpperCase()} Â· ${String(group.name || "").toUpperCase()} Â· ${group.fixtureCount} FIXTURE${group.fixtureCount === 1 ? "" : "S"} â”€â”€`)}">${group.teams.map((team) => { const value = `${group.key}|||${team}`; return `<option value="${this._escape(value)}" ${value === selected ? "selected" : ""}>${this._escape(team)}</option>`; }).join("")}</optgroup>`).join("")}`;
    const payer = game.players.find((player) => player.id === round.payerId);
  ...
  _lastManStandingPage() {
    ...
    return `
      ...
      ` : `
        ...
        ${competition.completed ? `<section class="page-card lms-winner"><ha-icon icon="mdi:trophy-award"></ha-icon><div><span class="eyebrow">COMPETITION COMPLETE</span><h2>${winner ? `${this._escape(winner.name)} wins!` : "No surviving player"}</h2><p>Final prize fund: Â£${prizeFund.toFixed(2)}</p></div></section>` : ""}
        ...
        <section class="lms-player-list">${competition.players.length ? [...competition.players].sort((a, b) => Number(a.paid) - Number(b.paid) || String(a.name).localeCompare(String(b.name))).map((player, playerIndex, sortedPlayers) => {
          ...
          const groupedOptions = teamGroups.map((league) => `<optgroup label="${this._escape(league.name)} Â· ${this._escape(league.teams.length)} teams">${league.teams.map((team) => `<option value="${this._escape(team)}" ${pick === team ? "selected" : ""} ${used.has(team) ? "disabled" : ""}>${this._escape(team)}${used.has(team) ? " Â· used" : ""}</option>`).join("")}</optgroup>`).join("");
          ...
          const earlyOptions = teamGroups.map((league) => `<optgroup label="${this._escape(league.name)} Â· ${this._escape(league.teams.length)} teams">${league.teams.map((team) => `<option value="${this._escape(team)}" ${earlyPick === team ? "selected" : ""} ${allUsedTeams.has(team) ? "disabled" : ""}>${this._escape(team)}${allUsedTeams.has(team) ? " Â· used" : ""}</option>`).join("")}</optgroup>`).join("");
          ...
          const statusText = statusClass === "eliminated" ? "Out" : boughtBack ? "Bought back Â· Through" : statusClass === "survived" ? "Through" : statusClass === "waiting" ? "Waiting for result Â· Still in" : statusClass === "selected" ? "Team selected Â· Still in" : "No team selected Â· Still in";
          ...
          return `${paymentHeading}<article class="page-card lms-player ${player.alive ? "alive" : "out"} ${statusClass}"><div class="lms-player-name"><ha-icon icon="${player.alive ? "mdi:shield-check-outline" : "mdi:close-octagon-outline"}"></ha-icon><div><strong>${this._escape(player.name)}</strong><span>Round ${competition.round}</span></div><span class="lms-payment-status ${player.paid ? "paid" : "unpaid"}">${player.paid ? `Â£ Paid${player.buyBacks ? " + buy-back" : ""}` : "Payment due"}</span>${adminUnlocked ? `<button class="lms-toggle-paid" data-player-id="${this._escape(player.id)}">${player.paid ? "Mark unpaid" : "Mark paid"}</button>` : ""}${adminUnlocked && player.pickUrl ? `<button class="lms-copy-player-link" data-player-id="${this._escape(player.id)}"><ha-icon icon="mdi:email-fast-outline"></ha-icon> Copy pick link</button>` : ""}${adminUnlocked && canBuyBack ? `<button class="lms-buy-back" data-player-id="${this._escape(player.id)}"><ha-icon icon="mdi:account-reactivate-outline"></ha-icon> Buy back Â£${entryFee.toFixed(2)}</button>` : ""}<span class="lms-pick-status ${statusClass}"><ha-icon icon="${statusIcon}"></ha-icon>${statusText}</span></div>${player.alive ? `<label><span>Round ${competition.round} pick</span><select class="lms-pick" data-player-id="${this._escape(player.id)}" ${pickLocked ? "disabled" : ""}><option value="">${deadline.locked ? "Picks locked" : !adminUnlocked ? "Administrator locked" : "Choose a team"}</option>${groupedOptions}</select></label>` : ""}${canPickEarly ? `<label class="lms-early-pick"><span><ha-icon icon="mdi:fast-forward-outline"></ha-icon> Through â€” choose Round ${Number(competition.round) + 1} early</span><select class="lms-next-pick" data-player-id="${this._escape(player.id)}" ${!adminUnlocked ? "disabled" : ""}><option value="">${!adminUnlocked ? "Administrator locked" : "Choose next-round team"}</option>${earlyOptions}</select></label>` : ""}<div class="lms-history">${Object.entries(player.picks || {}).map(([round, team]) => `<span>R${this._escape(round)} Â· ${this._escape(team)} Â· ${this._escape(player.results?.[round] || (round === nextRoundKey ? "early pick" : "pending"))}</span>`).join("") || `<span>No picks yet</span>`}</div><div class="lms-player-email"><ha-icon icon="mdi:email-lock-outline"></ha-icon><span>${this._escape(adminUnlocked ? (email || "No email added") : maskedEmail)}</span>${adminUnlocked ? `<div class="lms-player-admin-actions"><button class="lms-edit-player-email" data-player-id="${this._escape(player.id)}"><ha-icon icon="mdi:email-edit-outline"></ha-icon>${email ? "Edit email" : "Add email"}</button><button class="lms-remove-player" data-player-id="${this._escape(player.id)}"><ha-icon icon="mdi:account-remove-outline"></ha-icon>Remove player</button></div>` : `<small>Hidden until administrator unlocks</small>`}</div></article>`;
  ...
  _accaLeaguePage() {
    ...
    const fixtureBoard = `<section class="page-card acca-fixture-board"><header class="acca-section-head"><ha-icon icon="mdi:calendar-month-outline"></ha-icon><div><span class="eyebrow">FIXTURES AVAILABLE TO PICK</span><h2>${this._escape(this._doublePickRound()?.startDate || "")} to ${this._escape(this._doublePickRound()?.endDate || "")}</h2><p>Only the teams shown below appear in the player pick lists.</p></div><strong>${roundFixtures.length} matches</strong></header>${fixtureGroups.length ? `<div class="acca-fixture-groups">${fixtureGroups.map(({ competition, fixtures }) => `<section><div class="acca-fixture-group-title"><strong>${this._escape(competition.country)} Â· ${this._escape(competition.name)}</strong><span>${fixtures.length} match${fixtures.length === 1 ? "" : "es"}</span></div>${fixtures.map((fixture) => { const status = String(fixture.status_short || fixture.status || "").toUpperCase(); const finished = ["FT","AET","PEN","AWD","WO"].includes(status); const kickoff = this._lmsFixtureTimestamp(fixture); return `<article class="acca-fixture-row ${finished ? "finished" : "upcoming"}"><time>${kickoff ? new Date(kickoff * 1000).toLocaleString([], { weekday:"short", day:"numeric", month:"short", hour:"2-digit", minute:"2-digit" }) : "Time TBC"}</time><span class="home">${this._escape(fixture.home_team || "Home TBC")}</span><b>${finished ? `${fixture.home_goals ?? 0} - ${fixture.away_goals ?? 0}` : "v"}</b><span>${this._escape(fixture.away_team || "Away TBC")}</span><em>${finished ? "Finished" : status || "Scheduled"}</em></article>`; }).join("")}</section>`).join("")}</div>` : `<div class="empty"><strong>No fixtures loaded for these dates.</strong><span>Use Load fixtures and teams below for each selected competition.</span></div>`}</section>`;
    ...
    html = html.replace("TWO PICKS Â· 3/1/0 SCORING", `${game.pickCount || 2} PICKS Â· 3/1/0 SCORING`).replaceAll("/2 results", `/${game.pickCount || 2} results`);
    html = html.replace("<button id=\"dp-check-results\">", `<label>Stake Â£<input id="dp-stake" type="number" min="0" step="0.01" value="${Number(this._doublePickRound()?.stake || 0)}"></label><label>Return Â£<input id="dp-return" type="number" min="0" step="0.01" value="${Number(this._doublePickRound()?.returnAmount || 0)}"></label><button id="dp-email-reminders">Email pick reminder</button><button id="dp-email-confirmation">Email confirmed picks</button><button id="dp-email-results">Email results</button><button id="dp-check-results">`);
  ...
  _supportersPage() {
    ...
    return `
      ...
      <section class="support-hero page-card"><div><span class="support-kicker">ðŸº HELP BUILD THE FUTURE</span><h2>Support Football Hub</h2><p>This project started as a personal Home Assistant football dashboard and has grown thanks to community feedback, testing, ideas and support.</p><div class="support-actions"><a class="support-button primary" href="https://ko-fi.com/ady1984" target="_blank" rel="noopener noreferrer">â˜• Support via Ko-fi</a><a class="support-button" href="https://paypal.me/graffidoodle" target="_blank" rel="noopener noreferrer">ðŸ’³ Support via PayPal</a></div></div><div class="support-thanks"><strong>Thank you</strong><span>Every contribution helps</span></div></section>
      ...
      <section class="page-card premium-info"><div><span class="support-kicker">â­ PREMIUM SUPPORTERS</span><h2>Extra recognition inside Football Hub</h2><p>Donate Â£10 / $10 / â‚¬10 or more to be featured with your name, country flag and an optional personal message.</p></div><div class="premium-features"><span>ðŸ‘‘ Premium profile</span><span>ðŸŒ Name & country flag</span><span>ðŸ’¬ Personal message</span></div></section>
      <section class="support-summary page-card"><div><strong>${supporters.length}</strong><span>Total supporters</span></div><div><strong>${countries.length}</strong><span>Countries supporting</span></div><div><strong>${this._escape(latestDate || "â€”")}</strong><span>Latest support date</span></div></section>
  ...
  _processLiveNotifications() {
    ...
    matches.forEach((match) => {
      ...
      let title = "", body = `${match.home_team} ${this._score(match.home_goals)}â€“${this._score(match.away_goals)} ${match.away_team}`;
  ...
  _t(key) {
    const translations = {
      ...
      es:{overview:"Resumen",live:"En vivo",fixtures:"Partidos",results:"Resultados",table:"ClasificaciÃ³n",players:"Jugadores",myClub:"Mi club",supporters:"Seguidores",settings:"Ajustes",country:"PaÃ­s",league:"Liga",view:"Vista",desktop:"Escritorio",tablet:"Tableta",mobile:"MÃ³vil",language:"Idioma",chooseClub:"Elige tu club"},
      de:{overview:"Ãœbersicht",live:"Live",fixtures:"Spielplan",results:"Ergebnisse",table:"Tabelle",players:"Spieler",myClub:"Mein Verein",supporters:"UnterstÃ¼tzer",settings:"Einstellungen",country:"Land",league:"Liga",view:"Ansicht",desktop:"Desktop",tablet:"Tablet",mobile:"Mobil",language:"Sprache",chooseClub:"Verein auswÃ¤hlen"},
      ...
      fr:{overview:"AperÃ§u",live:"Direct",fixtures:"Matchs",results:"RÃ©sultats",table:"Classement",players:"Joueurs",myClub:"Mon club",supporters:"Supporters",settings:"RÃ©glages",country:"Pays",league:"Ligue",view:"Affichage",desktop:"Bureau",tablet:"Tablette",mobile:"Mobile",language:"Langue",chooseClub:"Choisissez votre club"},
      ...
      pt:{overview:"VisÃ£o geral",live:"Ao vivo",fixtures:"Jogos",results:"Resultados",table:"ClassificaÃ§Ã£o",players:"Jogadores",myClub:"Meu clube",supporters:"Apoiantes",settings:"DefiniÃ§Ãµes",country:"PaÃ­s",league:"Liga",view:"Vista",desktop:"Computador",tablet:"Tablet",mobile:"TelemÃ³vel",language:"Idioma",chooseClub:"Escolha o seu clube"},
      tr:{overview:"Genel BakÄ±ÅŸ",live:"CanlÄ±",fixtures:"FikstÃ¼r",results:"SonuÃ§lar",table:"Puan Durumu",players:"Oyuncular",myClub:"KulÃ¼bÃ¼m",supporters:"DestekÃ§iler",settings:"Ayarlar",country:"Ãœlke",league:"Lig",view:"GÃ¶rÃ¼nÃ¼m",desktop:"MasaÃ¼stÃ¼",tablet:"Tablet",mobile:"Mobil",language:"Dil",chooseClub:"KulÃ¼bÃ¼nÃ¼ seÃ§"},
    };
  ...
  _translateRenderedPage() {
    ...
    const phrases = {
      es:{"YOUR MATCHDAY STARTS HERE":"TU JORNADA EMPIEZA AQUÃ","Next fixture":"PrÃ³ximo partido","Live centre":"Centro en vivo","Latest result":"Ãšltimo resultado","Season":"Temporada","Total fixtures":"Partidos totales","League table":"ClasificaciÃ³n","Top scorers":"MÃ¡ximos goleadores","Top assists":"MÃ¡ximos asistentes","CURRENT STANDINGS":"CLASIFICACIÃ“N ACTUAL","CURRENT LIVE FEED":"SEÃ‘AL EN DIRECTO","Current live feed":"SeÃ±al en directo","The feed updates automatically when a match begins.":"La informaciÃ³n se actualiza automÃ¡ticamente cuando comienza un partido.","Supported team":"Equipo favorito","Choose a team":"Elige un equipo","No live matches right now.":"No hay partidos en directo ahora.","MATCH SCHEDULE":"CALENDARIO","Upcoming fixtures":"PrÃ³ximos partidos","Show fixtures for":"Mostrar partidos de","Next 6 fixtures":"PrÃ³ximos 6 partidos","COMPLETED MATCHES":"PARTIDOS FINALIZADOS","No results available yet.":"TodavÃ­a no hay resultados.","PLAYER LEADERBOARDS":"CLASIFICACIÃ“N DE JUGADORES","Player data is not available yet.":"Los datos de jugadores aÃºn no estÃ¡n disponibles.","YOUR TEAM CENTRE":"CENTRO DE TU EQUIPO","CLUB PROFILE":"PERFIL DEL CLUB","HOME GROUND":"ESTADIO","Club code":"CÃ³digo del club","Founded":"Fundado","City":"Ciudad","Capacity":"Capacidad","Surface":"Superficie","LEAGUE POSITION":"POSICIÃ“N EN LA LIGA","Played":"Jugados","Goal difference":"Diferencia de goles","Points":"Puntos","NEXT MATCH":"PRÃ“XIMO PARTIDO","SEASON RECORD":"REGISTRO DE TEMPORADA","Club statistics":"EstadÃ­sticas del club","Wins":"Victorias","Draws":"Empates","Defeats":"Derrotas","Goals scored":"Goles marcados","Clean sheets":"PorterÃ­as a cero","MANAGER":"ENTRENADOR","Age":"Edad","Career appointments":"Cargos anteriores","Recorded trophies":"Trofeos registrados","LATEST SCORES":"ÃšLTIMOS RESULTADOS","Recent results":"Resultados recientes","Club top scorers":"Goleadores del club","Club top assists":"Asistentes del club","FIRST TEAM":"PRIMER EQUIPO","Current squad":"Plantilla actual","AVAILABILITY":"DISPONIBILIDAD","Injuries & suspensions":"Lesiones y sanciones","TRANSFER CENTRE":"MERCADO DE FICHAJES","Recent transfers":"Fichajes recientes","Prediction":"PredicciÃ³n","Advice":"Consejo","Home chance":"Probabilidad local","Draw chance":"Probabilidad de empate","Away chance":"Probabilidad visitante","CLUB HISTORY":"HISTORIA DEL CLUB","Records":"Registros","Head-to-head matches":"Enfrentamientos directos","Player trophies":"Trofeos de jugadores","Manager trophies":"Trofeos del entrenador","COMMUNITY SUPPORT":"APOYO DE LA COMUNIDAD","Supporters around the world":"Seguidores de todo el mundo","Latest supporters":"Ãšltimos seguidores","All supporters":"Todos los seguidores","Total supporters":"Seguidores totales","Countries supporting":"PaÃ­ses participantes","Latest support date":"Ãšltima fecha de apoyo","FOOTBALL HUB":"FOOTBALL HUB","Settings & diagnostics":"Ajustes y diagnÃ³stico","Competition":"CompeticiÃ³n","Provider mode":"Modo de proveedor","Diagnostics":"DiagnÃ³stico","Integration":"IntegraciÃ³n","Configured competitions":"Competiciones configuradas","Panel build":"VersiÃ³n del panel","Entity prefix":"Prefijo de entidad","No fixtures available.":"No hay partidos disponibles.","No recent results available.":"No hay resultados recientes.","No transfer data available.":"No hay datos de fichajes.","No current injuries supplied.":"No hay lesiones actuales.","Squad information is not available yet.":"La plantilla aÃºn no estÃ¡ disponible.","Football Hub is ready":"Football Hub estÃ¡ listo"},
      de:{"YOUR MATCHDAY STARTS HERE":"DEIN SPIELTAG BEGINNT HIER","Next fixture":"NÃ¤chstes Spiel","Live centre":"Live-Zentrale","Latest result":"Letztes Ergebnis","Season":"Saison","Total fixtures":"Spiele gesamt","League table":"Tabelle","Top scorers":"TorschÃ¼tzen","Top assists":"Meiste Vorlagen","CURRENT STANDINGS":"AKTUELLE TABELLE","Current live feed":"Aktueller Live-Feed","The feed updates automatically when a match begins.":"Die Daten werden bei Spielbeginn automatisch aktualisiert.","Supported team":"Lieblingsverein","Choose a team":"Team auswÃ¤hlen","No live matches right now.":"Derzeit keine Live-Spiele.","MATCH SCHEDULE":"SPIELPLAN","Upcoming fixtures":"Kommende Spiele","Show fixtures for":"Spiele anzeigen fÃ¼r","Next 6 fixtures":"NÃ¤chste 6 Spiele","COMPLETED MATCHES":"BEENDETE SPIELE","No results available yet.":"Noch keine Ergebnisse verfÃ¼gbar.","PLAYER LEADERBOARDS":"SPIELER-RANGLISTEN","Player data is not available yet.":"Spielerdaten sind noch nicht verfÃ¼gbar.","YOUR TEAM CENTRE":"DEIN VEREINSZENTRUM","CLUB PROFILE":"VEREINSPROFIL","HOME GROUND":"HEIMSTADION","Club code":"VereinskÃ¼rzel","Founded":"GegrÃ¼ndet","City":"Stadt","Capacity":"KapazitÃ¤t","Surface":"SpielflÃ¤che","LEAGUE POSITION":"TABELLENPLATZ","Played":"Spiele","Goal difference":"Tordifferenz","Points":"Punkte","NEXT MATCH":"NÃ„CHSTES SPIEL","SEASON RECORD":"SAISONBILANZ","Club statistics":"Vereinsstatistik","Wins":"Siege","Draws":"Unentschieden","Defeats":"Niederlagen","Goals scored":"Erzielte Tore","Clean sheets":"Zu-null-Spiele","MANAGER":"TRAINER","Age":"Alter","Career appointments":"Karrierestationen","Recorded trophies":"Erfasste Titel","LATEST SCORES":"LETZTE ERGEBNISSE","Recent results":"Letzte Ergebnisse","Club top scorers":"Vereins-TorschÃ¼tzen","Club top assists":"Vereins-Vorlagen","FIRST TEAM":"ERSTE MANNSCHAFT","Current squad":"Aktueller Kader","AVAILABILITY":"VERFÃœGBARKEIT","Injuries & suspensions":"Verletzungen und Sperren","TRANSFER CENTRE":"TRANSFERZENTRALE","Recent transfers":"Letzte Transfers","Prediction":"Prognose","Advice":"Empfehlung","Home chance":"Heimchance","Draw chance":"Remischance","Away chance":"AuswÃ¤rtschance","CLUB HISTORY":"VEREINSHISTORIE","Records":"Rekorde","Head-to-head matches":"Direkte Duelle","Player trophies":"Spielertitel","Manager trophies":"Trainertitel","COMMUNITY SUPPORT":"COMMUNITY-UNTERSTÃœTZUNG","Supporters around the world":"UnterstÃ¼tzer weltweit","Latest supporters":"Neueste UnterstÃ¼tzer","All supporters":"Alle UnterstÃ¼tzer","Total supporters":"UnterstÃ¼tzer gesamt","Countries supporting":"UnterstÃ¼tzende LÃ¤nder","Latest support date":"Letztes UnterstÃ¼tzungsdatum","Settings & diagnostics":"Einstellungen und Diagnose","Competition":"Wettbewerb","Provider mode":"Anbietermodus","Diagnostics":"Diagnose","Integration":"Integration","Configured competitions":"Konfigurierte Wettbewerbe","Panel build":"Panel-Version","Entity prefix":"EntitÃ¤tsprÃ¤fix","No fixtures available.":"Keine Spiele verfÃ¼gbar.","No recent results available.":"Keine aktuellen Ergebnisse.","No transfer data available.":"Keine Transferdaten verfÃ¼gbar.","No current injuries supplied.":"Keine aktuellen Verletzungen gemeldet.","Squad information is not available yet.":"Kaderdaten sind noch nicht verfÃ¼gbar.","Football Hub is ready":"Football Hub ist bereit"},
      fr:{"YOUR MATCHDAY STARTS HERE":"VOTRE JOURNÃ‰E COMMENCE ICI","Next fixture":"Prochain match","Live centre":"Centre en direct","Latest result":"Dernier rÃ©sultat","Season":"Saison","Total fixtures":"Total des matchs","League table":"Classement","Top scorers":"Meilleurs buteurs","Top assists":"Meilleurs passeurs","CURRENT STANDINGS":"CLASSEMENT ACTUEL","Current live feed":"Direct actuel","The feed updates automatically when a match begins.":"Les donnÃ©es se mettent Ã  jour automatiquement au dÃ©but du match.","Supported team":"Ã‰quipe favorite","Choose a team":"Choisir une Ã©quipe","No live matches right now.":"Aucun match en direct actuellement.","MATCH SCHEDULE":"CALENDRIER","Upcoming fixtures":"Prochains matchs","Show fixtures for":"Afficher les matchs de","Next 6 fixtures":"6 prochains matchs","COMPLETED MATCHES":"MATCHS TERMINÃ‰S","No results available yet.":"Aucun rÃ©sultat disponible.","PLAYER LEADERBOARDS":"CLASSEMENTS DES JOUEURS","Player data is not available yet.":"Les donnÃ©es des joueurs ne sont pas encore disponibles.","YOUR TEAM CENTRE":"CENTRE DE VOTRE Ã‰QUIPE","CLUB PROFILE":"PROFIL DU CLUB","HOME GROUND":"STADE","Club code":"Code du club","Founded":"FondÃ©","City":"Ville","Capacity":"CapacitÃ©","Surface":"Surface","LEAGUE POSITION":"POSITION EN CHAMPIONNAT","Played":"JouÃ©s","Goal difference":"DiffÃ©rence de buts","Points":"Points","NEXT MATCH":"PROCHAIN MATCH","SEASON RECORD":"BILAN DE SAISON","Club statistics":"Statistiques du club","Wins":"Victoires","Draws":"Nuls","Defeats":"DÃ©faites","Goals scored":"Buts marquÃ©s","Clean sheets":"Matchs sans encaisser","MANAGER":"ENTRAÃŽNEUR","Age":"Ã‚ge","Career appointments":"Postes occupÃ©s","Recorded trophies":"TrophÃ©es enregistrÃ©s","LATEST SCORES":"DERNIERS RÃ‰SULTATS","Recent results":"RÃ©sultats rÃ©cents","Club top scorers":"Buteurs du club","Club top assists":"Passeurs du club","FIRST TEAM":"Ã‰QUIPE PREMIÃˆRE","Current squad":"Effectif actuel","AVAILABILITY":"DISPONIBILITÃ‰","Injuries & suspensions":"Blessures et suspensions","TRANSFER CENTRE":"MERCATO","Recent transfers":"Transferts rÃ©cents","Prediction":"Pronostic","Advice":"Conseil","Home chance":"Chance domicile","Draw chance":"Chance de nul","Away chance":"Chance extÃ©rieur","CLUB HISTORY":"HISTOIRE DU CLUB","Records":"Records","Head-to-head matches":"Confrontations directes","Player trophies":"TrophÃ©es des joueurs","Manager trophies":"TrophÃ©es de l'entraÃ®neur","COMMUNITY SUPPORT":"SOUTIEN DE LA COMMUNAUTÃ‰","Supporters around the world":"Supporters dans le monde","Latest supporters":"Derniers supporters","All supporters":"Tous les supporters","Total supporters":"Total des supporters","Countries supporting":"Pays reprÃ©sentÃ©s","Latest support date":"DerniÃ¨re date de soutien","Settings & diagnostics":"RÃ©glages et diagnostic","Competition":"CompÃ©tition","Provider mode":"Mode fournisseur","Diagnostics":"Diagnostic","Integration":"IntÃ©gration","Configured competitions":"CompÃ©titions configurÃ©es","Panel build":"Version du panneau","Entity prefix":"PrÃ©fixe d'entitÃ©","No fixtures available.":"Aucun match disponible.","No recent results available.":"Aucun rÃ©sultat rÃ©cent.","No transfer data available.":"Aucune donnÃ©e de transfert.","No current injuries supplied.":"Aucune blessure actuelle signalÃ©e.","Squad information is not available yet.":"L'effectif n'est pas encore disponible.","Football Hub is ready":"Football Hub est prÃªt"},
      it:{"YOUR MATCHDAY STARTS HERE":"LA TUA GIORNATA INIZIA QUI","Next fixture":"Prossima partita","Live centre":"Centro live","Latest result":"Ultimo risultato","Season":"Stagione","Total fixtures":"Partite totali","League table":"Classifica","Top scorers":"Capocannonieri","Top assists":"Migliori assist","CURRENT STANDINGS":"CLASSIFICA ATTUALE","Current live feed":"Diretta attuale","Supported team":"Squadra preferita","Choose a team":"Scegli una squadra","No live matches right now.":"Nessuna partita in diretta.","MATCH SCHEDULE":"CALENDARIO","Upcoming fixtures":"Prossime partite","Show fixtures for":"Mostra partite di","Next 6 fixtures":"Prossime 6 partite","COMPLETED MATCHES":"PARTITE TERMINATE","No results available yet.":"Nessun risultato disponibile.","PLAYER LEADERBOARDS":"CLASSIFICHE GIOCATORI","Player data is not available yet.":"Dati giocatori non ancora disponibili.","YOUR TEAM CENTRE":"CENTRO DELLA TUA SQUADRA","CLUB PROFILE":"PROFILO CLUB","HOME GROUND":"STADIO","Club code":"Codice club","Founded":"Fondato","City":"CittÃ ","Capacity":"Capienza","Surface":"Superficie","LEAGUE POSITION":"POSIZIONE IN CLASSIFICA","Played":"Giocate","Goal difference":"Differenza reti","Points":"Punti","NEXT MATCH":"PROSSIMA PARTITA","SEASON RECORD":"RENDIMENTO STAGIONALE","Club statistics":"Statistiche club","Wins":"Vittorie","Draws":"Pareggi","Defeats":"Sconfitte","Goals scored":"Gol segnati","Clean sheets":"Porte inviolate","MANAGER":"ALLENATORE","Age":"EtÃ ","Career appointments":"Incarichi in carriera","Recorded trophies":"Trofei registrati","LATEST SCORES":"ULTIMI RISULTATI","Recent results":"Risultati recenti","Club top scorers":"Marcatori del club","Club top assists":"Assist del club","FIRST TEAM":"PRIMA SQUADRA","Current squad":"Rosa attuale","AVAILABILITY":"DISPONIBILITÃ€","Injuries & suspensions":"Infortuni e squalifiche","TRANSFER CENTRE":"CALCIOMERCATO","Recent transfers":"Trasferimenti recenti","Prediction":"Pronostico","Advice":"Consiglio","Home chance":"ProbabilitÃ  casa","Draw chance":"ProbabilitÃ  pareggio","Away chance":"ProbabilitÃ  trasferta","CLUB HISTORY":"STORIA DEL CLUB","Records":"Record","Head-to-head matches":"Scontri diretti","Player trophies":"Trofei giocatori","Manager trophies":"Trofei allenatore","Supporters around the world":"Sostenitori nel mondo","Latest supporters":"Ultimi sostenitori","All supporters":"Tutti i sostenitori","Total supporters":"Sostenitori totali","Countries supporting":"Paesi rappresentati","Settings & diagnostics":"Impostazioni e diagnostica","Competition":"Competizione","Diagnostics":"Diagnostica","Configured competitions":"Competizioni configurate","Panel build":"Versione pannello","No fixtures available.":"Nessuna partita disponibile.","No recent results available.":"Nessun risultato recente.","No transfer data available.":"Nessun dato sui trasferimenti.","No current injuries supplied.":"Nessun infortunio attuale.","Squad information is not available yet.":"La rosa non Ã¨ ancora disponibile."},
      ...
      pt:{"YOUR MATCHDAY STARTS HERE":"O TEU DIA DE JOGO COMEÃ‡A AQUI","Next fixture":"PrÃ³ximo jogo","Live centre":"Centro ao vivo","Latest result":"Ãšltimo resultado","Season":"Ã‰poca","Total fixtures":"Total de jogos","League table":"ClassificaÃ§Ã£o","Top scorers":"Melhores marcadores","Top assists":"Mais assistÃªncias","CURRENT STANDINGS":"CLASSIFICAÃ‡ÃƒO ATUAL","Current live feed":"Direto atual","Supported team":"Equipa favorita","Choose a team":"Escolha uma equipa","No live matches right now.":"NÃ£o hÃ¡ jogos em direto.","MATCH SCHEDULE":"CALENDÃRIO","Upcoming fixtures":"PrÃ³ximos jogos","Show fixtures for":"Mostrar jogos de","Next 6 fixtures":"PrÃ³ximos 6 jogos","COMPLETED MATCHES":"JOGOS TERMINADOS","No results available yet.":"Ainda nÃ£o hÃ¡ resultados.","PLAYER LEADERBOARDS":"CLASSIFICAÃ‡Ã•ES DE JOGADORES","Player data is not available yet.":"Os dados dos jogadores ainda nÃ£o estÃ£o disponÃ­veis.","YOUR TEAM CENTRE":"CENTRO DA TUA EQUIPA","CLUB PROFILE":"PERFIL DO CLUBE","HOME GROUND":"ESTÃDIO","Club code":"CÃ³digo do clube","Founded":"Fundado","City":"Cidade","Capacity":"Capacidade","Surface":"SuperfÃ­cie","LEAGUE POSITION":"POSIÃ‡ÃƒO NA LIGA","Played":"Jogos","Goal difference":"DiferenÃ§a de golos","Points":"Pontos","NEXT MATCH":"PRÃ“XIMO JOGO","SEASON RECORD":"REGISTO DA Ã‰POCA","Club statistics":"EstatÃ­sticas do clube","Wins":"VitÃ³rias","Draws":"Empates","Defeats":"Derrotas","Goals scored":"Golos marcados","Clean sheets":"Jogos sem sofrer","MANAGER":"TREINADOR","Age":"Idade","Career appointments":"Cargos na carreira","Recorded trophies":"TrofÃ©us registados","LATEST SCORES":"ÃšLTIMOS RESULTADOS","Recent results":"Resultados recentes","Club top scorers":"Marcadores do clube","Club top assists":"AssistÃªncias do clube","FIRST TEAM":"EQUIPA PRINCIPAL","Current squad":"Plantel atual","AVAILABILITY":"DISPONIBILIDADE","Injuries & suspensions":"LesÃµes e suspensÃµes","TRANSFER CENTRE":"CENTRO DE TRANSFERÃŠNCIAS","Recent transfers":"TransferÃªncias recentes","Prediction":"PrevisÃ£o","Advice":"Conselho","Home chance":"HipÃ³tese da casa","Draw chance":"HipÃ³tese de empate","Away chance":"HipÃ³tese visitante","CLUB HISTORY":"HISTÃ“RIA DO CLUBE","Records":"Recordes","Head-to-head matches":"Confrontos diretos","Player trophies":"TrofÃ©us dos jogadores","Manager trophies":"TrofÃ©us do treinador","Supporters around the world":"Apoiantes em todo o mundo","Latest supporters":"Apoiantes recentes","All supporters":"Todos os apoiantes","Total supporters":"Total de apoiantes","Countries supporting":"PaÃ­ses representados","Settings & diagnostics":"DefiniÃ§Ãµes e diagnÃ³stico","Competition":"CompetiÃ§Ã£o","Diagnostics":"DiagnÃ³stico","Configured competitions":"CompetiÃ§Ãµes configuradas","Panel build":"VersÃ£o do painel","No fixtures available.":"NÃ£o hÃ¡ jogos disponÃ­veis.","No recent results available.":"NÃ£o hÃ¡ resultados recentes.","No transfer data available.":"NÃ£o hÃ¡ dados de transferÃªncias.","No current injuries supplied.":"NÃ£o hÃ¡ lesÃµes atuais.","Squad information is not available yet.":"O plantel ainda nÃ£o estÃ¡ disponÃ­vel."},
      tr:{"YOUR MATCHDAY STARTS HERE":"MAÃ‡ GÃœNÃœN BURADA BAÅžLIYOR","Next fixture":"SÄ±radaki maÃ§","Live centre":"CanlÄ± merkez","Latest result":"Son sonuÃ§","Season":"Sezon","Total fixtures":"Toplam maÃ§","League table":"Puan durumu","Top scorers":"Gol krallÄ±ÄŸÄ±","Top assists":"Asist krallÄ±ÄŸÄ±","CURRENT STANDINGS":"GÃœNCEL PUAN DURUMU","Current live feed":"GÃ¼ncel canlÄ± yayÄ±n","Supported team":"Desteklenen takÄ±m","Choose a team":"TakÄ±m seÃ§","No live matches right now.":"Åžu anda canlÄ± maÃ§ yok.","MATCH SCHEDULE":"MAÃ‡ PROGRAMI","Upcoming fixtures":"YaklaÅŸan maÃ§lar","Show fixtures for":"MaÃ§larÄ± gÃ¶ster","Next 6 fixtures":"SÄ±radaki 6 maÃ§","COMPLETED MATCHES":"TAMAMLANAN MAÃ‡LAR","No results available yet.":"HenÃ¼z sonuÃ§ yok.","PLAYER LEADERBOARDS":"OYUNCU SIRALAMALARI","Player data is not available yet.":"Oyuncu verileri henÃ¼z mevcut deÄŸil.","YOUR TEAM CENTRE":"TAKIM MERKEZÄ°N","CLUB PROFILE":"KULÃœP PROFÄ°LÄ°","HOME GROUND":"STADYUM","Club code":"KulÃ¼p kodu","Founded":"KuruluÅŸ","City":"Åžehir","Capacity":"Kapasite","Surface":"Zemin","LEAGUE POSITION":"LÄ°G SIRASI","Played":"Oynanan","Goal difference":"Averaj","Points":"Puan","NEXT MATCH":"SIRADAKÄ° MAÃ‡","SEASON RECORD":"SEZON KARNESÄ°","Club statistics":"KulÃ¼p istatistikleri","Wins":"Galibiyet","Draws":"Beraberlik","Defeats":"MaÄŸlubiyet","Goals scored":"AtÄ±lan gol","Clean sheets":"Gol yemeden","MANAGER":"TEKNÄ°K DÄ°REKTÃ–R","Age":"YaÅŸ","Career appointments":"Kariyer gÃ¶revleri","Recorded trophies":"KayÄ±tlÄ± kupalar","LATEST SCORES":"SON SONUÃ‡LAR","Recent results":"Son sonuÃ§lar","Club top scorers":"KulÃ¼p golcÃ¼leri","Club top assists":"KulÃ¼p asistleri","FIRST TEAM":"A TAKIM","Current squad":"GÃ¼ncel kadro","AVAILABILITY":"UYGUNLUK","Injuries & suspensions":"SakatlÄ±klar ve cezalar","TRANSFER CENTRE":"TRANSFER MERKEZÄ°","Recent transfers":"Son transferler","Prediction":"Tahmin","Advice":"Ã–neri","Home chance":"Ev sahibi ÅŸansÄ±","Draw chance":"Beraberlik ÅŸansÄ±","Away chance":"Deplasman ÅŸansÄ±","CLUB HISTORY":"KULÃœP TARÄ°HÄ°","Records":"Rekorlar","Head-to-head matches":"Ä°kili rekabet","Player trophies":"Oyuncu kupalarÄ±","Manager trophies":"Teknik direktÃ¶r kupalarÄ±","Supporters around the world":"DÃ¼nyadaki destekÃ§iler","Latest supporters":"Son destekÃ§iler","All supporters":"TÃ¼m destekÃ§iler","Total supporters":"Toplam destekÃ§i","Countries supporting":"Destek veren Ã¼lkeler","Settings & diagnostics":"Ayarlar ve tanÄ±lama","Competition":"Organizasyon","Diagnostics":"TanÄ±lama","Configured competitions":"YapÄ±landÄ±rÄ±lan organizasyonlar","Panel build":"Panel sÃ¼rÃ¼mÃ¼","No fixtures available.":"MaÃ§ bulunmuyor.","No recent results available.":"SonuÃ§ bulunmuyor.","No transfer data available.":"Transfer verisi yok.","No current injuries supplied.":"GÃ¼ncel sakatlÄ±k bildirilmedi.","Squad information is not available yet.":"Kadro bilgisi henÃ¼z mevcut deÄŸil."},
    };
    const supporterTranslations = {
      es:{"COMMUNITY SUPPORT":"APOYO DE LA COMUNIDAD","Supporters":"Seguidores","HELP BUILD THE FUTURE":"AYUDA A CONSTRUIR EL FUTURO","Support Football Hub":"Apoya Football Hub","This project started as a personal Home Assistant football dashboard and has grown thanks to community feedback, testing, ideas and support.":"Este proyecto comenzÃ³ como un panel personal de fÃºtbol para Home Assistant y ha crecido gracias a los comentarios, las pruebas, las ideas y el apoyo de la comunidad.","Support via Ko-fi":"Apoyar mediante Ko-fi","Support via PayPal":"Apoyar mediante PayPal","Thank you":"Gracias","Every contribution helps":"Cada contribuciÃ³n ayuda","Live data costs":"Costes de datos en directo","Helps cover reliable football data and matchday services.":"Ayuda a cubrir datos de fÃºtbol fiables y servicios de jornada.","Fixes & improvements":"Correcciones y mejoras","Supports testing, maintenance and regular updates.":"Apoya las pruebas, el mantenimiento y las actualizaciones periÃ³dicas.","Future features":"Funciones futuras","Helps build more competitions, statistics and dashboard tools.":"Ayuda a crear mÃ¡s competiciones, estadÃ­sticas y herramientas para el panel.","Community development":"Desarrollo comunitario","Keeps the project community-led and available to Home Assistant users.":"Mantiene el proyecto dirigido por la comunidad y disponible para los usuarios de Home Assistant.","PREMIUM SUPPORTERS":"SEGUIDORES PREMIUM","Extra recognition inside Football Hub":"Reconocimiento adicional dentro de Football Hub","Premium profile":"Perfil premium","Name & country flag":"Nombre y bandera del paÃ­s","Personal message":"Mensaje personal","LATEST THANK YOUS":"ÃšLTIMOS AGRADECIMIENTOS","THE COMMUNITY":"LA COMUNIDAD","To be added as a supporter":"Para aparecer como seguidor","After donating, include your name, country and optional short message.":"DespuÃ©s de donar, incluye tu nombre, paÃ­s y un mensaje breve opcional.","Support the project and have your name added here as a thank you.":"Apoya el proyecto y aÃ±adiremos tu nombre aquÃ­ como agradecimiento.","No supporters loaded yet":"TodavÃ­a no hay seguidores cargados"},
      de:{"COMMUNITY SUPPORT":"COMMUNITY-UNTERSTÃœTZUNG","Supporters":"UnterstÃ¼tzer","HELP BUILD THE FUTURE":"HILF, DIE ZUKUNFT ZU GESTALTEN","Support Football Hub":"Football Hub unterstÃ¼tzen","This project started as a personal Home Assistant football dashboard and has grown thanks to community feedback, testing, ideas and support.":"Dieses Projekt begann als persÃ¶nliches Home-Assistant-FuÃŸball-Dashboard und ist dank Feedback, Tests, Ideen und UnterstÃ¼tzung der Community gewachsen.","Support via Ko-fi":"Ãœber Ko-fi unterstÃ¼tzen","Support via PayPal":"Ãœber PayPal unterstÃ¼tzen","Thank you":"Danke","Every contribution helps":"Jeder Beitrag hilft","Live data costs":"Kosten fÃ¼r Live-Daten","Helps cover reliable football data and matchday services.":"Hilft, zuverlÃ¤ssige FuÃŸballdaten und Spieltagsdienste zu finanzieren.","Fixes & improvements":"Korrekturen und Verbesserungen","Supports testing, maintenance and regular updates.":"UnterstÃ¼tzt Tests, Wartung und regelmÃ¤ÃŸige Updates.","Future features":"ZukÃ¼nftige Funktionen","Helps build more competitions, statistics and dashboard tools.":"Hilft beim Ausbau von Wettbewerben, Statistiken und Dashboard-Werkzeugen.","Community development":"Community-Entwicklung","Keeps the project community-led and available to Home Assistant users.":"HÃ¤lt das Projekt communitygefÃ¼hrt und fÃ¼r Home-Assistant-Nutzer verfÃ¼gbar.","PREMIUM SUPPORTERS":"PREMIUM-UNTERSTÃœTZER","Extra recognition inside Football Hub":"ZusÃ¤tzliche Anerkennung in Football Hub","Premium profile":"Premium-Profil","Name & country flag":"Name und Landesflagge","Personal message":"PersÃ¶nliche Nachricht","LATEST THANK YOUS":"NEUESTE DANKSAGUNGEN","THE COMMUNITY":"DIE COMMUNITY","To be added as a supporter":"Als UnterstÃ¼tzer aufgenommen werden","After donating, include your name, country and optional short message.":"Gib nach der Spende deinen Namen, dein Land und optional eine kurze Nachricht an."},
      fr:{"COMMUNITY SUPPORT":"SOUTIEN DE LA COMMUNAUTÃ‰","Supporters":"Supporters","HELP BUILD THE FUTURE":"AIDEZ Ã€ CONSTRUIRE Lâ€™AVENIR","Support Football Hub":"Soutenir Football Hub","This project started as a personal Home Assistant football dashboard and has grown thanks to community feedback, testing, ideas and support.":"Ce projet a commencÃ© comme un tableau de bord de football personnel pour Home Assistant et a grandi grÃ¢ce aux retours, tests, idÃ©es et soutien de la communautÃ©.","Support via Ko-fi":"Soutenir via Ko-fi","Support via PayPal":"Soutenir via PayPal","Thank you":"Merci","Every contribution helps":"Chaque contribution compte","Live data costs":"CoÃ»ts des donnÃ©es en direct","Helps cover reliable football data and matchday services.":"Aide Ã  financer des donnÃ©es fiables et les services de jour de match.","Fixes & improvements":"Corrections et amÃ©liorations","Supports testing, maintenance and regular updates.":"Soutient les tests, la maintenance et les mises Ã  jour rÃ©guliÃ¨res.","Future features":"Fonctions futures","Helps build more competitions, statistics and dashboard tools.":"Aide Ã  crÃ©er davantage de compÃ©titions, statistiques et outils.","Community development":"DÃ©veloppement communautaire","Keeps the project community-led and available to Home Assistant users.":"Maintient le projet dirigÃ© par la communautÃ© et disponible pour les utilisateurs de Home Assistant.","PREMIUM SUPPORTERS":"SUPPORTERS PREMIUM","Extra recognition inside Football Hub":"Reconnaissance supplÃ©mentaire dans Football Hub","Premium profile":"Profil premium","Name & country flag":"Nom et drapeau du pays","Personal message":"Message personnel","LATEST THANK YOUS":"DERNIERS REMERCIEMENTS","THE COMMUNITY":"LA COMMUNAUTÃ‰","To be added as a supporter":"Pour Ãªtre ajoutÃ© comme supporter","After donating, include your name, country and optional short message.":"AprÃ¨s votre don, indiquez votre nom, votre pays et un court message facultatif."},
      it:{"COMMUNITY SUPPORT":"SUPPORTO DELLA COMUNITÃ€","Supporters":"Sostenitori","HELP BUILD THE FUTURE":"AIUTA A COSTRUIRE IL FUTURO","Support Football Hub":"Sostieni Football Hub","This project started as a personal Home Assistant football dashboard and has grown thanks to community feedback, testing, ideas and support.":"Questo progetto Ã¨ nato come dashboard personale di calcio per Home Assistant ed Ã¨ cresciuto grazie a feedback, test, idee e supporto della comunitÃ .","Support via Ko-fi":"Sostieni con Ko-fi","Support via PayPal":"Sostieni con PayPal","Thank you":"Grazie","Every contribution helps":"Ogni contributo aiuta","Live data costs":"Costi dei dati live","Helps cover reliable football data and matchday services.":"Aiuta a coprire dati calcistici affidabili e servizi partita.","Fixes & improvements":"Correzioni e miglioramenti","Supports testing, maintenance and regular updates.":"Supporta test, manutenzione e aggiornamenti regolari.","Future features":"Funzioni future","Helps build more competitions, statistics and dashboard tools.":"Aiuta a creare piÃ¹ competizioni, statistiche e strumenti.","Community development":"Sviluppo della comunitÃ ","Keeps the project community-led and available to Home Assistant users.":"Mantiene il progetto guidato dalla comunitÃ  e disponibile per gli utenti Home Assistant.","PREMIUM SUPPORTERS":"SOSTENITORI PREMIUM","Extra recognition inside Football Hub":"Riconoscimento extra in Football Hub","Premium profile":"Profilo premium","Name & country flag":"Nome e bandiera","Personal message":"Messaggio personale","LATEST THANK YOUS":"ULTIMI RINGRAZIAMENTI","THE COMMUNITY":"LA COMUNITÃ€","To be added as a supporter":"Per essere aggiunto come sostenitore","After donating, include your name, country and optional short message.":"Dopo la donazione, indica nome, paese e un breve messaggio facoltativo."},
      nl:{"COMMUNITY SUPPORT":"COMMUNITYSTEUN","Supporters":"Supporters","HELP BUILD THE FUTURE":"HELP DE TOEKOMST BOUWEN","Support Football Hub":"Steun Football Hub","This project started as a personal Home Assistant football dashboard and has grown thanks to community feedback, testing, ideas and support.":"Dit project begon als een persoonlijk Home Assistant-voetbaldashboard en groeide dankzij feedback, tests, ideeÃ«n en steun van de community.","Support via Ko-fi":"Steun via Ko-fi","Support via PayPal":"Steun via PayPal","Thank you":"Bedankt","Every contribution helps":"Elke bijdrage helpt","Live data costs":"Kosten voor livegegevens","Helps cover reliable football data and matchday services.":"Helpt betrouwbare voetbalgegevens en wedstrijddiensten te betalen.","Fixes & improvements":"Oplossingen en verbeteringen","Supports testing, maintenance and regular updates.":"Ondersteunt tests, onderhoud en regelmatige updates.","Future features":"Toekomstige functies","Helps build more competitions, statistics and dashboard tools.":"Helpt meer competities, statistieken en hulpmiddelen te bouwen.","Community development":"Communityontwikkeling","Keeps the project community-led and available to Home Assistant users.":"Houdt het project communitygestuurd en beschikbaar voor Home Assistant-gebruikers.","PREMIUM SUPPORTERS":"PREMIUM SUPPORTERS","Extra recognition inside Football Hub":"Extra erkenning in Football Hub","Premium profile":"Premiumprofiel","Name & country flag":"Naam en landenvlag","Personal message":"Persoonlijk bericht","LATEST THANK YOUS":"NIEUWSTE BEDANKJES","THE COMMUNITY":"DE COMMUNITY","To be added as a supporter":"Om als supporter toegevoegd te worden","After donating, include your name, country and optional short message.":"Vermeld na het doneren je naam, land en eventueel een kort bericht."},
      pt:{"COMMUNITY SUPPORT":"APOIO DA COMUNIDADE","Supporters":"Apoiantes","HELP BUILD THE FUTURE":"AJUDE A CONSTRUIR O FUTURO","Support Football Hub":"Apoiar Football Hub","This project started as a personal Home Assistant football dashboard and has grown thanks to community feedback, testing, ideas and support.":"Este projeto comeÃ§ou como um painel pessoal de futebol para Home Assistant e cresceu graÃ§as ao feedback, testes, ideias e apoio da comunidade.","Support via Ko-fi":"Apoiar via Ko-fi","Support via PayPal":"Apoiar via PayPal","Thank you":"Obrigado","Every contribution helps":"Cada contribuiÃ§Ã£o ajuda","Live data costs":"Custos de dados em direto","Helps cover reliable football data and matchday services.":"Ajuda a financiar dados fiÃ¡veis e serviÃ§os de jogo.","Fixes & improvements":"CorreÃ§Ãµes e melhorias","Supports testing, maintenance and regular updates.":"Apoia testes, manutenÃ§Ã£o e atualizaÃ§Ãµes regulares.","Future features":"Funcionalidades futuras","Helps build more competitions, statistics and dashboard tools.":"Ajuda a criar mais competiÃ§Ãµes, estatÃ­sticas e ferramentas.","Community development":"Desenvolvimento comunitÃ¡rio","Keeps the project community-led and available to Home Assistant users.":"MantÃ©m o projeto liderado pela comunidade e disponÃ­vel para utilizadores Home Assistant.","PREMIUM SUPPORTERS":"APOIANTES PREMIUM","Extra recognition inside Football Hub":"Reconhecimento adicional no Football Hub","Premium profile":"Perfil premium","Name & country flag":"Nome e bandeira do paÃ­s","Personal message":"Mensagem pessoal","LATEST THANK YOUS":"AGRADECIMENTOS RECENTES","THE COMMUNITY":"A COMUNIDADE","To be added as a supporter":"Para ser adicionado como apoiante","After donating, include your name, country and optional short message.":"ApÃ³s doar, indique o seu nome, paÃ­s e uma mensagem curta opcional."},
      tr:{"COMMUNITY SUPPORT":"TOPLULUK DESTEÄžÄ°","Supporters":"DestekÃ§iler","HELP BUILD THE FUTURE":"GELECEÄžÄ° KURMAYA YARDIM ET","Support Football Hub":"Football Hubâ€™Ä± destekle","This project started as a personal Home Assistant football dashboard and has grown thanks to community feedback, testing, ideas and support.":"Bu proje kiÅŸisel bir Home Assistant futbol paneli olarak baÅŸladÄ± ve topluluk geri bildirimi, testleri, fikirleri ve desteÄŸiyle bÃ¼yÃ¼dÃ¼.","Support via Ko-fi":"Ko-fi ile destekle","Support via PayPal":"PayPal ile destekle","Thank you":"TeÅŸekkÃ¼rler","Every contribution helps":"Her katkÄ± yardÄ±mcÄ± olur","Live data costs":"CanlÄ± veri maliyetleri","Helps cover reliable football data and matchday services.":"GÃ¼venilir futbol verileri ve maÃ§ gÃ¼nÃ¼ hizmetlerinin maliyetini karÅŸÄ±lamaya yardÄ±mcÄ± olur.","Fixes & improvements":"DÃ¼zeltmeler ve iyileÅŸtirmeler","Supports testing, maintenance and regular updates.":"Testleri, bakÄ±mÄ± ve dÃ¼zenli gÃ¼ncellemeleri destekler.","Future features":"Gelecek Ã¶zellikler","Helps build more competitions, statistics and dashboard tools.":"Daha fazla lig, istatistik ve panel aracÄ± oluÅŸturmaya yardÄ±mcÄ± olur.","Community development":"Topluluk geliÅŸtirmesi","Keeps the project community-led and available to Home Assistant users.":"Projeyi topluluk odaklÄ± ve Home Assistant kullanÄ±cÄ±larÄ±na aÃ§Ä±k tutar.","PREMIUM SUPPORTERS":"PREMÄ°UM DESTEKÃ‡Ä°LER","Extra recognition inside Football Hub":"Football Hub iÃ§inde ek gÃ¶rÃ¼nÃ¼rlÃ¼k","Premium profile":"Premium profil","Name & country flag":"Ad ve Ã¼lke bayraÄŸÄ±","Personal message":"KiÅŸisel mesaj","LATEST THANK YOUS":"SON TEÅžEKKÃœRLER","THE COMMUNITY":"TOPLULUK","To be added as a supporter":"DestekÃ§i olarak eklenmek iÃ§in","After donating, include your name, country and optional short message.":"BaÄŸÄ±ÅŸtan sonra adÄ±nÄ±zÄ±, Ã¼lkenizi ve isteÄŸe baÄŸlÄ± kÄ±sa mesajÄ±nÄ±zÄ± belirtin."},
    };
    ...
    const completePagePhrases = {
      "Cups":["Copas","Pokale","Coupes","Coppe","Bekers","TaÃ§as","Kupalar"],
      "News":["Noticias","Nachrichten","ActualitÃ©s","Notizie","Nieuws","NotÃ­cias","Haberler"],
      "TV Guide":["GuÃ­a TV","TV-Programm","Programme TV","Guida TV","TV-gids","Guia TV","TV Rehberi"],
      "Transfers":["Fichajes","Transfers","Transferts","Trasferimenti","Transfers","TransferÃªncias","Transferler"],
      "Football News":["Noticias de fÃºtbol","FuÃŸballnachrichten","ActualitÃ©s football","Notizie di calcio","Voetbalnieuws","NotÃ­cias de futebol","Futbol Haberleri"],
      "Transfer Market":["Mercado de fichajes","Transfermarkt","MarchÃ© des transferts","Calciomercato","Transfermarkt","Mercado de transferÃªncias","Transfer PazarÄ±"],
      "Latest stories":["Ãšltimas noticias","Neueste Meldungen","DerniÃ¨res actualitÃ©s","Ultime notizie","Laatste verhalen","Ãšltimas notÃ­cias","Son haberler"],
      "Latest transfers":["Ãšltimos fichajes","Neueste Transfers","Derniers transferts","Ultimi trasferimenti","Laatste transfers","TransferÃªncias recentes","Son transferler"],
      "Top transfers":["Principales fichajes","Top-Transfers","Principaux transferts","Trasferimenti principali","Toptransfers","Principais transferÃªncias","En bÃ¼yÃ¼k transferler"],
      "Transfer centre":["Mercado de fichajes","Transferzentrale","Centre des transferts","Centro trasferimenti","Transfercentrum","Centro de transferÃªncias","Transfer merkezi"],
      "LIVE AROUND THE WORLD":["DIRECTO EN TODO EL MUNDO","LIVE WELTWEIT","DIRECT DANS LE MONDE","LIVE NEL MONDO","LIVE WERELDWIJD","DIRETO EM TODO O MUNDO","DÃœNYADAN CANLI"],
      "All live scores":["Todos los marcadores en vivo","Alle Live-Ergebnisse","Tous les scores en direct","Tutti i risultati live","Alle live-scores","Todos os resultados em direto","TÃ¼m canlÄ± skorlar"],
      "Live now":["En directo","Jetzt live","En direct","In diretta","Nu live","Em direto","CanlÄ±"],
      "Goals":["Goles","Tore","Buts","Gol","Doelpunten","Golos","Goller"],
      "Selected events":["Eventos seleccionados","AusgewÃ¤hlte Ereignisse","Ã‰vÃ©nements sÃ©lectionnÃ©s","Eventi selezionati","Geselecteerde gebeurtenissen","Eventos selecionados","SeÃ§ili olaylar"],
      "MATCHDAY CONTROL ROOM":["CENTRO DE CONTROL DEL PARTIDO","SPIELTAG-KONTROLLZENTRALE","CENTRE DE CONTRÃ”LE DU MATCH","SALA DI CONTROLLO PARTITA","WEDSTRIJDCENTRALE","CENTRO DE CONTROLO DO JOGO","MAÃ‡ KONTROL MERKEZÄ°"],
      "Scores, incidents, statistics and team sheets update automatically.":["Los marcadores, incidentes, estadÃ­sticas y alineaciones se actualizan automÃ¡ticamente.","Ergebnisse, Ereignisse, Statistiken und Aufstellungen werden automatisch aktualisiert.","Les scores, incidents, statistiques et compositions sont actualisÃ©s automatiquement.","Risultati, eventi, statistiche e formazioni si aggiornano automaticamente.","Scores, gebeurtenissen, statistieken en opstellingen worden automatisch bijgewerkt.","Resultados, incidentes, estatÃ­sticas e equipas atualizam automaticamente.","Skorlar, olaylar, istatistikler ve kadrolar otomatik gÃ¼ncellenir."],
      "Match timeline":["CronologÃ­a del partido","Spielverlauf","Chronologie du match","Cronologia partita","Wedstrijdverloop","Cronologia do jogo","MaÃ§ zaman Ã§izelgesi"],
      "Statistics":["EstadÃ­sticas","Statistiken","Statistiques","Statistiche","Statistieken","EstatÃ­sticas","Ä°statistikler"],
      "Match statistics":["EstadÃ­sticas del partido","Spielstatistiken","Statistiques du match","Statistiche partita","Wedstrijdstatistieken","EstatÃ­sticas do jogo","MaÃ§ istatistikleri"],
      "Starting line-ups":["Alineaciones iniciales","Startaufstellungen","Compositions de dÃ©part","Formazioni iniziali","Basisopstellingen","Equipas iniciais","Ä°lk 11'ler"],
      "Statistics not available yet.":["Las estadÃ­sticas aÃºn no estÃ¡n disponibles.","Statistiken sind noch nicht verfÃ¼gbar.","Les statistiques ne sont pas encore disponibles.","Le statistiche non sono ancora disponibili.","Statistieken zijn nog niet beschikbaar.","As estatÃ­sticas ainda nÃ£o estÃ£o disponÃ­veis.","Ä°statistikler henÃ¼z mevcut deÄŸil."],
      "Line-ups not available yet.":["Las alineaciones aÃºn no estÃ¡n disponibles.","Aufstellungen sind noch nicht verfÃ¼gbar.","Les compositions ne sont pas encore disponibles.","Le formazioni non sono ancora disponibili.","Opstellingen zijn nog niet beschikbaar.","As equipas ainda nÃ£o estÃ£o disponÃ­veis.","Kadrolar henÃ¼z mevcut deÄŸil."],
      "No match events yet.":["AÃºn no hay eventos del partido.","Noch keine Spielereignisse.","Aucun Ã©vÃ©nement de match pour le moment.","Nessun evento partita.","Nog geen wedstrijdgebeurtenissen.","Ainda nÃ£o hÃ¡ eventos do jogo.","HenÃ¼z maÃ§ olayÄ± yok."],
      "Most appearances":["MÃ¡s partidos","Meiste EinsÃ¤tze","Plus d'apparitions","PiÃ¹ presenze","Meeste wedstrijden","Mais jogos","En Ã§ok forma giyenler"],
      "Most minutes played":["MÃ¡s minutos jugados","Meiste Spielminuten","Plus de minutes jouÃ©es","PiÃ¹ minuti giocati","Meeste speelminuten","Mais minutos jogados","En Ã§ok dakika"],
      "Top yellow cards":["MÃ¡s tarjetas amarillas","Meiste gelbe Karten","Plus de cartons jaunes","PiÃ¹ cartellini gialli","Meeste gele kaarten","Mais cartÃµes amarelos","En Ã§ok sarÄ± kart"],
      "Top red cards":["MÃ¡s tarjetas rojas","Meiste rote Karten","Plus de cartons rouges","PiÃ¹ cartellini rossi","Meeste rode kaarten","Mais cartÃµes vermelhos","En Ã§ok kÄ±rmÄ±zÄ± kart"],
      "Top ratings":["Mejores valoraciones","Beste Bewertungen","Meilleures notes","Migliori valutazioni","Beste beoordelingen","Melhores avaliaÃ§Ãµes","En yÃ¼ksek puanlar"],
      "CUP COMPETITIONS":["COMPETICIONES DE COPA","POKALWETTBEWERBE","COMPÃ‰TITIONS DE COUPE","COMPETIZIONI DI COPPA","BEKERCOMPETITIES","COMPETIÃ‡Ã•ES DE TAÃ‡A","KUPA ORGANÄ°ZASYONLARI"],
      "Choose a country, then select the cup you want Football Hub to follow.":["Elige un paÃ­s y despuÃ©s la copa que quieres seguir.","WÃ¤hle ein Land und anschlieÃŸend den gewÃ¼nschten Pokal.","Choisissez un pays, puis la coupe Ã  suivre.","Scegli un paese, poi la coppa da seguire.","Kies een land en daarna de beker die je wilt volgen.","Escolha um paÃ­s e depois a taÃ§a que pretende seguir.","Bir Ã¼lke ve ardÄ±ndan takip edilecek kupayÄ± seÃ§in."],
      "Country / region":["PaÃ­s / regiÃ³n","Land / Region","Pays / rÃ©gion","Paese / regione","Land / regio","PaÃ­s / regiÃ£o","Ãœlke / bÃ¶lge"],
      "Choose a competition":["Elige una competiciÃ³n","Wettbewerb auswÃ¤hlen","Choisir une compÃ©tition","Scegli una competizione","Kies een competitie","Escolha uma competiÃ§Ã£o","Organizasyon seÃ§"],
      "SELECTED COMPETITION":["COMPETICIÃ“N SELECCIONADA","AUSGEWÃ„HLTER WETTBEWERB","COMPÃ‰TITION SÃ‰LECTIONNÃ‰E","COMPETIZIONE SELEZIONATA","GESELECTEERDE COMPETITIE","COMPETIÃ‡ÃƒO SELECIONADA","SEÃ‡Ä°LÄ° ORGANÄ°ZASYON"],
      "Table / phase standings":["Tabla / clasificaciÃ³n de fase","Tabelle / Phasenstand","Classement / phase","Classifica / fase","Stand / fase","Tabela / classificaÃ§Ã£o da fase","Tablo / aÅŸama sÄ±ralamasÄ±"],
      "Knockout competition":["CompeticiÃ³n eliminatoria","K.-o.-Wettbewerb","CompÃ©tition Ã  Ã©limination directe","Competizione a eliminazione","Knock-outcompetitie","CompetiÃ§Ã£o a eliminar","Eleme organizasyonu"],
      "Open":["Abrir","Ã–ffnen","Ouvrir","Apri","Openen","Abrir","AÃ§"],
      "View all":["Ver todo","Alle anzeigen","Tout afficher","Vedi tutto","Alles bekijken","Ver tudo","TÃ¼mÃ¼nÃ¼ gÃ¶r"],
      ...
      "View full fixtures":["Ver todos los partidos","Alle Spiele","Voir tous les matchs","Tutte le partite","Alle wedstrijden","Ver todos os jogos","TÃ¼m fikstÃ¼r"],
      "View all results":["Ver todos los resultados","Alle Ergebnisse","Voir tous les rÃ©sultats","Tutti i risultati","Alle uitslagen","Ver todos os resultados","TÃ¼m sonuÃ§lar"],
      "View all players":["Ver todos los jugadores","Alle Spieler","Voir tous les joueurs","Tutti i giocatori","Alle spelers","Ver todos os jogadores","TÃ¼m oyuncular"],
      "View all news":["Ver todas las noticias","Alle Nachrichten","Voir toutes les actualitÃ©s","Tutte le notizie","Al het nieuws","Ver todas as notÃ­cias","TÃ¼m haberler"],
      "Channel to be confirmed":["Canal por confirmar","Sender noch offen","ChaÃ®ne Ã  confirmer","Canale da confermare","Zender nog niet bevestigd","Canal por confirmar","Kanal henÃ¼z belli deÄŸil"],
      "To be confirmed":["Por confirmar","Noch offen","Ã€ confirmer","Da confermare","Nog te bevestigen","Por confirmar","HenÃ¼z belli deÄŸil"],
      "Venue TBC":["Estadio por confirmar","Spielort noch offen","Stade Ã  confirmer","Stadio da confermare","Stadion nog te bevestigen","EstÃ¡dio por confirmar","Stadyum belli deÄŸil"],
      "Referee TBC":["Ãrbitro por confirmar","Schiedsrichter noch offen","Arbitre Ã  confirmer","Arbitro da confermare","Scheidsrechter nog te bevestigen","Ãrbitro por confirmar","Hakem belli deÄŸil"],
      "Online":["En lÃ­nea","Online","En ligne","Online","Online","Online","Ã‡evrimiÃ§i"],
      "Substitution":["SustituciÃ³n","Auswechslung","Remplacement","Sostituzione","Wissel","SubstituiÃ§Ã£o","Oyuncu deÄŸiÅŸikliÄŸi"],
      "Half-time":["Descanso","Halbzeit","Mi-temps","Intervallo","Rust","Intervalo","Ä°lk yarÄ±"],
      "Full-time":["Final","Abpfiff","Fin du match","Fine partita","Afgelopen","Fim do jogo","MaÃ§ sonu"],
      "Grass":["CÃ©sped","Rasen","Pelouse","Erba","Gras","Relva","Ã‡im"],
      "Mostly Cloudy":["Mayormente nublado","Ãœberwiegend bewÃ¶lkt","GÃ©nÃ©ralement nuageux","Prevalentemente nuvoloso","Overwegend bewolkt","Maioritariamente nublado","Ã‡oÄŸunlukla bulutlu"],
      "Clear":["Despejado","Klar","DÃ©gagÃ©","Sereno","Helder","CÃ©u limpo","AÃ§Ä±k"],
      "Rain":["Lluvia","Regen","Pluie","Pioggia","Regen","Chuva","YaÄŸmur"],
      "Ball possession":["PosesiÃ³n","Ballbesitz","Possession","Possesso palla","Balbezit","Posse de bola","Topa sahip olma"],
      "Total shots":["Tiros totales","SchÃ¼sse gesamt","Tirs totaux","Tiri totali","Totaal schoten","Remates totais","Toplam ÅŸut"],
      "Shots on target":["Tiros a puerta","SchÃ¼sse aufs Tor","Tirs cadrÃ©s","Tiri in porta","Schoten op doel","Remates Ã  baliza","Ä°sabetli ÅŸut"],
      "Shots off target":["Tiros fuera","SchÃ¼sse daneben","Tirs non cadrÃ©s","Tiri fuori","Schoten naast","Remates para fora","Ä°sabetsiz ÅŸut"],
      "Blocked shots":["Tiros bloqueados","Geblockte SchÃ¼sse","Tirs bloquÃ©s","Tiri respinti","Geblokkeerde schoten","Remates bloqueados","Engellenen ÅŸut"],
      "Shots inside box":["Tiros dentro del Ã¡rea","SchÃ¼sse im Strafraum","Tirs dans la surface","Tiri in area","Schoten in het strafschopgebied","Remates na Ã¡rea","Ceza sahasÄ± iÃ§inden ÅŸut"],
      "Shots outside box":["Tiros fuera del Ã¡rea","SchÃ¼sse auÃŸerhalb","Tirs hors surface","Tiri fuori area","Schoten buiten het strafschopgebied","Remates fora da Ã¡rea","Ceza sahasÄ± dÄ±ÅŸÄ±ndan ÅŸut"],
      "Corners":["CÃ³rners","Ecken","Corners","Calci d'angolo","Corners","Cantos","Kornerler"],
      "Fouls":["Faltas","Fouls","Fautes","Falli","Overtredingen","Faltas","Fauller"],
      "Yellow cards":["Tarjetas amarillas","Gelbe Karten","Cartons jaunes","Cartellini gialli","Gele kaarten","CartÃµes amarelos","SarÄ± kartlar"],
      "Red cards":["Tarjetas rojas","Rote Karten","Cartons rouges","Cartellini rossi","Rode kaarten","CartÃµes vermelhos","KÄ±rmÄ±zÄ± kartlar"],
      "Passes":["Pases","PÃ¤sse","Passes","Passaggi","Passes","Passes","Paslar"],
      "Accurate passes":["Pases precisos","Erfolgreiche PÃ¤sse","Passes rÃ©ussies","Passaggi riusciti","Nauwkeurige passes","Passes certos","Ä°sabetli paslar"],
      "No TV listings are available right now.":["No hay programaciÃ³n de TV disponible.","Derzeit ist kein TV-Programm verfÃ¼gbar.","Aucun programme TV n'est disponible actuellement.","Nessuna programmazione TV disponibile.","Er zijn momenteel geen tv-uitzendingen beschikbaar.","NÃ£o hÃ¡ programaÃ§Ã£o televisiva disponÃ­vel.","Åžu anda TV yayÄ±nÄ± yok."],
      "News is loading.":["Cargando noticias.","Nachrichten werden geladen.","Chargement des actualitÃ©s.","Caricamento notizie.","Nieuws wordt geladen.","A carregar notÃ­cias.","Haberler yÃ¼kleniyor."],
      "Transfer data is loading.":["Cargando fichajes.","Transferdaten werden geladen.","Chargement des transferts.","Caricamento trasferimenti.","Transfergegevens worden geladen.","A carregar transferÃªncias.","Transfer verileri yÃ¼kleniyor."],
      "Fee undisclosed":["Importe no revelado","AblÃ¶se unbekannt","Montant non communiquÃ©","Costo non comunicato","Bedrag niet bekendgemaakt","Valor nÃ£o divulgado","Ãœcret aÃ§Ä±klanmadÄ±"],
      "On loan":["Cedido","Ausgeliehen","En prÃªt","In prestito","Gehuurd","Por emprÃ©stimo","KiralÄ±k"],
      "Your fixtures, results, league position and players will appear here.":["Tus partidos, resultados, posiciÃ³n y jugadores aparecerÃ¡n aquÃ­.","Deine Spiele, Ergebnisse, Tabellenposition und Spieler erscheinen hier.","Vos matchs, rÃ©sultats, classement et joueurs apparaÃ®tront ici.","Partite, risultati, posizione e giocatori appariranno qui.","Je wedstrijden, resultaten, positie en spelers verschijnen hier.","Os seus jogos, resultados, posiÃ§Ã£o e jogadores aparecerÃ£o aqui.","FikstÃ¼r, sonuÃ§lar, lig sÄ±rasÄ± ve oyuncular burada gÃ¶rÃ¼necek."],
      "Stadium information":["InformaciÃ³n del estadio","Stadioninformationen","Informations sur le stade","Informazioni stadio","Stadioninformatie","InformaÃ§Ãµes do estÃ¡dio","Stadyum bilgileri"],
      "Opened":["Inaugurado","ErÃ¶ffnet","Ouvert","Inaugurato","Geopend","Inaugurado","AÃ§Ä±lÄ±ÅŸ"],
      "Seasons recorded":["Temporadas registradas","Erfasste Saisons","Saisons enregistrÃ©es","Stagioni registrate","Geregistreerde seizoenen","Ã‰pocas registadas","KayÄ±tlÄ± sezonlar"],
      "Losses":["Derrotas","Niederlagen","DÃ©faites","Sconfitte","Nederlagen","Derrotas","MaÄŸlubiyetler"],
      "Points per game":["Puntos por partido","Punkte pro Spiel","Points par match","Punti per partita","Punten per wedstrijd","Pontos por jogo","MaÃ§ baÅŸÄ±na puan"],
      "Total trophies":["Trofeos totales","Titel gesamt","Total des trophÃ©es","Trofei totali","Totaal prijzen","Total de trofÃ©us","Toplam kupa"],
      "Competitions won":["Competiciones ganadas","Gewonnene Wettbewerbe","CompÃ©titions remportÃ©es","Competizioni vinte","Gewonnen competities","CompetiÃ§Ãµes ganhas","KazanÄ±lan organizasyonlar"],
      "League seasons recorded":["Temporadas de liga registradas","Erfasste Ligasaisons","Saisons de championnat enregistrÃ©es","Stagioni di campionato registrate","Geregistreerde competitieseizoenen","Ã‰pocas de liga registadas","KayÄ±tlÄ± lig sezonlarÄ±"],
      "Buy me a beer":["InvÃ­tame a una cerveza","Spendiere mir ein Bier","Offrez-moi une biÃ¨re","Offrimi una birra","Trakteer me op een biertje","Pague-me uma cerveja","Bana bir bira Ä±smarla"],
      "Enjoying Football Hub?":["Â¿Te gusta Football Hub?","GefÃ¤llt dir Football Hub?","Vous aimez Football Hub ?","Ti piace Football Hub?","Bevalt Football Hub?","Gosta do Football Hub?","Football Hub'Ä± beÄŸendiniz mi?"],
      "Help support its development and buy me a beer.":["Ayuda a su desarrollo e invÃ­tame a una cerveza.","UnterstÃ¼tze die Entwicklung und spendiere mir ein Bier.","Soutenez son dÃ©veloppement et offrez-moi une biÃ¨re.","Sostieni lo sviluppo e offrimi una birra.","Steun de ontwikkeling en trakteer me op een biertje.","Apoie o desenvolvimento e pague-me uma cerveja.","GeliÅŸimi destekleyin ve bana bir bira Ä±smarlayÄ±n."],
      "Football Hub Â· Built for Home Assistant":["Football Hub Â· Creado para Home Assistant","Football Hub Â· FÃ¼r Home Assistant entwickelt","Football Hub Â· ConÃ§u pour Home Assistant","Football Hub Â· Creato per Home Assistant","Football Hub Â· Gebouwd voor Home Assistant","Football Hub Â· Criado para Home Assistant","Football Hub Â· Home Assistant iÃ§in geliÅŸtirildi"]
    };
    ...
    while ((node = walker.nextNode())) {
      ...
      const decorated = trimmed.match(/^([^A-Za-zÃ€-Å¾]*)(.+)$/);
      ...
      if (countMatch) {
        const countWords = {
          ...
          fr:{matches:"matchs",moves:"transferts",stories:"actualitÃ©s",players:"joueurs",live:"en direct"},
          ...
          pt:{matches:"jogos",moves:"transferÃªncias",stories:"notÃ­cias",players:"jogadores",live:"em direto"},
          tr:{matches:"maÃ§",moves:"transfer",stories:"haber",players:"oyuncu",live:"canlÄ±"}
        };
      ...
      if (capacityMatch) {
        const word = {es:"capacidad",de:"KapazitÃ¤t",fr:"places",it:"posti",nl:"capaciteit",pt:"capacidade",tr:"kapasite"}[this._language];
  ...
  _displayCountry(country) {
    ...
    const special = {
      England:{es:"Inglaterra",de:"England",fr:"Angleterre",it:"Inghilterra",nl:"Engeland",pt:"Inglaterra",tr:"Ä°ngiltere"},
      Scotland:{es:"Escocia",de:"Schottland",fr:"Ã‰cosse",it:"Scozia",nl:"Schotland",pt:"EscÃ³cia",tr:"Ä°skoÃ§ya"},
      Wales:{es:"Gales",de:"Wales",fr:"Pays de Galles",it:"Galles",nl:"Wales",pt:"PaÃ­s de Gales",tr:"Galler"},
      "Northern Ireland":{es:"Irlanda del Norte",de:"Nordirland",fr:"Irlande du Nord",it:"Irlanda del Nord",nl:"Noord-Ierland",pt:"Irlanda do Norte",tr:"Kuzey Ä°rlanda"},
      "Republic of Ireland":{es:"Irlanda",de:"Irland",fr:"Irlande",it:"Irlanda",nl:"Ierland",pt:"Irlanda",tr:"Ä°rlanda"},
      USA:{es:"Estados Unidos",de:"USA",fr:"Ã‰tats-Unis",it:"Stati Uniti",nl:"Verenigde Staten",pt:"Estados Unidos",tr:"ABD"},
  ...
  _score(value) {
    return value === null || value === undefined ? "â€“" : this._escape(value);
  }
  ...
  _matchCard(match, mode = "fixture") {
    ...
    const statusLabels = { NS: "Upcoming", TBD: "Time TBC", "1H": "Live Â· First half", HT: "Half-time", "2H": "Live Â· Second half", ET: "Extra-time", BT: "Extra-time break", P: "Penalties", FT: "Full-time", AET: "After extra-time", PEN: "After penalties", PST: "Postponed", CANC: "Cancelled", ABD: "Abandoned", SUSP: "Suspended", AWD: "Awarded", WO: "Walkover" };
    ...
    const score = isResult || match.status_short !== "NS"
      ? `<div class="match-score">${this._score(match.home_goals)} <span>â€“</span> ${this._score(match.away_goals)}</div>`
    ...
    return `
      <header class="hero">
        <div>
          ...
          <p>${this._escape(activeCompetition.name || "Choose a competition")} Â· ${this._escape(
      status.season || ""
    )}</p></div></div>
        </div>
        <div class="hero-actions">
          <label class="view-mode-picker language-picker"><span>${this._t("language")}</span><select id="language-select" aria-label="Language">
            <option value="en" ${this._language === "en" ? "selected" : ""}>English</option><option value="es" ${this._language === "es" ? "selected" : ""}>EspaÃ±ol</option><option value="de" ${this._language === "de" ? "selected" : ""}>Deutsch</option><option value="it" ${this._language === "it" ? "selected" : ""}>Italiano</option><option value="fr" ${this._language === "fr" ? "selected" : ""}>FranÃ§ais</option><option value="nl" ${this._language === "nl" ? "selected" : ""}>Nederlands</option><option value="pt" ${this._language === "pt" ? "selected" : ""}>PortuguÃªs</option><option value="tr" ${this._language === "tr" ? "selected" : ""}>TÃ¼rkÃ§e</option>
          </select></label>
          ...
          <a class="header-beer-link" href="https://paypal.me/graffidoodle" target="_blank" rel="noopener noreferrer" title="Buy me a beer" aria-label="Buy me a beer">
            <span class="beer-icon">ðŸº</span>
          </a>
  ...
  _overview() {
    ...
    return `
          ...
          ${
            last.home_team
              ? `<div class="latest-result">
                  ...
                  <strong>${this._score(last.home_goals)} â€“ ${this._score(last.away_goals)}</strong>
      ...
      <section class="overview-beer page-card">
        <span class="overview-beer-icon">ðŸº</span>
  ...
  _eventRows(events) {
    ...
    return `<div class="event-timeline">${orderedEvents.map((event) => {
      ...
      const icon = type.toLowerCase().includes("goal") ? "âš½" :
        type.toLowerCase().includes("card") ? (detail.toLowerCase().includes("red") ? "ðŸŸ¥" : "ðŸŸ¨") :
        type.toLowerCase().includes("subst") ? "ðŸ”„" : "â—";
      return `<div class="event-row">
        <span class="event-minute">${this._escape(elapsed)}${this._escape(extra)}'</span>
        <span class="event-icon">${icon}</span>
        <span class="event-copy"><strong>${this._escape(title)}</strong><small>${this._escape(subtitle)}${eventText(event.assist) ? ` Â· Assist: ${this._escape(eventText(event.assist))}` : ""}</small></span>
      </div>`;
    }).join("")}</div>`;
  }
  ...
  _statRows(statistics, live) {
    ...
    const displayValue = (value) => {
      ...
      if (typeof value === "string" || typeof value === "number") return String(value);
      if (Array.isArray(value)) return value.map(displayValue).filter((item) => item !== "â€“").join(" / ") || "â€“";
    ...
    return `<div class="lineup-grid">${lineups.map((lineup) => {
      ...
      return `<div class="team-sheet">
        ...
        <div class="starting-xi">${starters.map((entry) => {
          ...
          return `<div><span>${this._escape(player.number ?? "â€“")}</span><strong>${this._escape(player.name || "Player")}</strong><small>${this._escape(player.pos || player.position || "")}</small></div>`;
        }).join("")}</div>
      </div>`;
    }).join("")}</div>`;
  }
      ...
      <article class="country-live-group">
        ...
        <div>${games.map((match, index) => {
          ...
          return `<button class="country-live-row ${id === selectedId ? "active" : ""}" data-live-score="${this._escape(id)}">
            ...
            <strong>${this._score(match.home_goals)}<i>â€“</i>${this._score(match.away_goals)}</strong>
  ...
  _livePage() {
    ...
    const countryCodes = { ENG:"England", SCO:"Scotland", WAL:"Wales", NIR:"Northern Ireland", IRL:"Ireland", GBR:"United Kingdom", USA:"United States", BRA:"Brazil", ARG:"Argentina", ESP:"Spain", GER:"Germany", ITA:"Italy", FRA:"France", NED:"Netherlands", POR:"Portugal", BEL:"Belgium", TUR:"TÃ¼rkiye", MEX:"Mexico", CAN:"Canada", AUS:"Australia", JPN:"Japan", KOR:"South Korea", CHN:"China", IND:"India", AUT:"Austria", SUI:"Switzerland", DEN:"Denmark", SWE:"Sweden", NOR:"Norway", FIN:"Finland", POL:"Poland", CZE:"Czech Republic", GRE:"Greece", CRO:"Croatia", SRB:"Serbia", UKR:"Ukraine", RUS:"Russia", ROU:"Romania", BUL:"Bulgaria", HUN:"Hungary", ISR:"Israel", KSA:"Saudi Arabia", UAE:"United Arab Emirates", QAT:"Qatar", EGY:"Egypt", MAR:"Morocco", RSA:"South Africa", NGA:"Nigeria", COL:"Colombia", CHI:"Chile", URU:"Uruguay", PAR:"Paraguay", ECU:"Ecuador", PER:"Peru", VEN:"Venezuela", INT:"International", FIFA:"International", UEFA:"Europe" };
    ...
    if (!primary.is_live) {
      return `
        ...
        <section class="page-card live-control-empty">
          <div><span class="live-kicker">âš½ MATCHDAY CONTROL ROOM</span><h2>Live Centre</h2><p>The feed updates automatically when a match begins.</p></div>
          ${teamOptions()}
    ...
    return `
      ...
      <section class="page-card live-control-hero">
        <div><span class="live-kicker">âš½ MATCHDAY CONTROL ROOM</span><h2>Live Centre <b>LIVE</b></h2><p>Scores, incidents, statistics and team sheets update automatically.</p></div>
      ...
      <section class="live-centre-card" id="selected-live-match"><button type="button" id="live-close-match" class="live-close-match"><ha-icon icon="mdi:arrow-left"></ha-icon> All live matches</button>
        <div class="live-banner"><span class="pulse"></span> LIVE Â· ${this._escape(
        ...
        <div class="live-matchup">
          ...
          <div class="score-board">
            <strong>${this._score(live.home_goals)} â€“ ${this._score(live.away_goals)}</strong>
        ...
        <div class="live-details">
          ...
          ${live.weather ? `<span><ha-icon icon="mdi:weather-partly-cloudy"></ha-icon>${this._escape(live.weather)}${live.temperature != null ? ` Â· ${this._escape(live.temperature)}Â°C` : ""}</span>` : ""}
      ...
      <section class="section">
        ...
        ${filteredFixtures.length > pageSize ? `
          <div class="fixture-pagination">
            ...
            <span>Page ${currentPage + 1} of ${totalPages} Â· ${filteredFixtures.length} matches</span>
        ...
        <div class="favourite-club-list">
          ...
          ${favourites.length ? favourites.map((item) => `
            <span class="favourite-club-chip">
              ...
              <small>${this._escape(item.country || "")} Â· ${this._escape(item.competition || "")}</small>
              <button type="button" class="favourite-club-remove" data-team="${this._escape(item.team)}" data-competition="${this._escape(item.competition)}" title="Remove favourite">Ã—</button>
      ...
      ${!club ? `<section class="page-card centred"><ha-icon class="huge-icon" icon="mdi:shield-star-outline"></ha-icon><h2>Choose your club</h2><p>Your fixtures, results, league position and players will appear here.</p></section>` : `
        <section class="two-column">
          <article class="page-card club-profile-card">
            <div class="club-profile-head">${this._logo(clubProfile.logo || standing?.team_logo, club, "86")}<div><span class="eyebrow">CLUB PROFILE</span><h2>${this._escape(clubProfile.name || club)}</h2><p>${this._escape(clubProfile.country || this._statusInfo().country || "")} ${clubProfile.founded ? `Â· Founded ${this._escape(clubProfile.founded)}` : ""}</p></div></div>
            <div class="settings-list"><div><span>Club code</span><strong>${this._escape(clubProfile.code || "â€”")}</strong></div><div><span>Founded</span><strong>${this._escape(clubProfile.founded || "â€”")}</strong></div><div><span>Country</span><strong>${this._escape(clubProfile.country || "â€”")}</strong></div></div>
          </article>
          <article class="page-card venue-card">
            ...
            <div class="settings-list"><div><span>City</span><strong>${this._escape(venue.city || "â€”")}</strong></div><div><span>Country</span><strong>${this._escape(venue.country || clubProfile.country || "â€”")}</strong></div><div><span>Capacity</span><strong>${this._escape(venue.capacity ? Number(venue.capacity).toLocaleString() : "â€”")}</strong></div><div><span>Opened</span><strong>${this._escape(venue.opened || "â€”")}</strong></div><div><span>Surface</span><strong>${this._escape(venue.surface || "â€”")}</strong></div></div>
          </article>
        </section>
        <section class="two-column">
          <article class="page-card"><span class="eyebrow">LEAGUE POSITION</span><h2>${standing ? `${this._escape(standing.rank)} Â· ${this._escape(club)}` : this._escape(club)}</h2>${standing ? `<div class="settings-list"><div><span>Played</span><strong>${this._escape(standing.played ?? standing.all?.played ?? 0)}</strong></div><div><span>Goal difference</span><strong>${this._escape(standing.goals_diff ?? standing.goal_difference ?? 0)}</strong></div><div><span>Points</span><strong>${this._escape(standing.points ?? 0)}</strong></div></div>` : `<div class="empty">League position is not available yet.</div>`}</article>
        ...
        <section class="two-column">
          ...
          <article class="page-card"><span class="eyebrow">MANAGER</span><div class="club-profile-head">${this._logo(coach.photo, coach.name, "72")}<div><h2>${this._escape(coach.name || "Manager unavailable")}</h2><p>${this._escape(coach.nationality || coach.current_season || "")}</p></div></div><div class="settings-list"><div><span>Age</span><strong>${this._escape(coach.age || "â€”")}</strong></div><div><span>Seasons recorded</span><strong>${this._escape(coach.career?.length || 0)}</strong></div><div><span>Wins</span><strong>${this._escape(coach.wins ?? "â€”")}</strong></div><div><span>Draws</span><strong>${this._escape(coach.draws ?? "â€”")}</strong></div><div><span>Losses</span><strong>${this._escape(coach.losses ?? "â€”")}</strong></div><div><span>Points per game</span><strong>${this._escape(coach.points_per_game ?? "â€”")}</strong></div></div></article>
        </section>
        ...
        <section class="section"><div class="section-title-row"><div><span class="eyebrow">FIRST TEAM</span><h3>Current squad</h3></div><span>${squad.length} players</span></div><div class="squad-grid">${squad.length ? squad.map((player) => `<article class="page-card squad-player">${this._logo(player.photo, player.name, "54")}<div><strong>${this._escape(player.name || "Player")}</strong><small>${this._escape([player.number ? `#${player.number}` : "", player.position, player.nationality].filter(Boolean).join(" Â· "))}</small><small>${this._escape([player.age ? `Age ${player.age}` : "", player.height ? `${player.height} cm` : "", money(player.transfer_value)].filter(Boolean).join(" Â· "))}</small>${player.injured ? `<em>Injured${player.expected_return ? ` Â· ${this._escape(player.expected_return)}` : ""}</em>` : ""}</div></article>`).join("") : `<div class="empty">Squad information is not available yet.</div>`}</div></section>
        <section class="two-column">
          ...
          <article class="page-card"><span class="eyebrow">TRANSFER CENTRE</span><h2>Recent transfers</h2><div class="player-list">${transfers.length ? transfers.slice(0, 10).map((item) => { const fee = item.fee_display || money(item.fee_value) || item.fee || (item.on_loan ? "On loan" : item.type) || "Fee undisclosed"; return `<div class="transfer-row">${this._logo(item.player?.photo, item.player?.name, "40")}<span class="player-name"><strong>${this._escape(item.player?.name || "Player")}</strong><small>${this._escape(`${item.teams?.out?.name || "Unknown"} â†’ ${item.teams?.in?.name || "Unknown"}`)}</small></span><span class="transfer-meta"><strong class="transfer-fee">${this._escape(fee)}</strong><time>${this._escape(String(item.date || "").slice(0, 10))}</time></span></div>`; }).join("") : `<div class="empty">No transfer data available.</div>`}</div></article>
        </section>
        <section class="two-column">
          <article class="page-card"><span class="eyebrow">NEXT MATCH</span><h2>Prediction</h2><div class="settings-list"><div><span>Advice</span><strong>${this._escape(prediction.predictions?.advice || "Not available")}</strong></div><div><span>Home chance</span><strong>${this._escape(prediction.predictions?.percent?.home || "â€”")}</strong></div><div><span>Draw chance</span><strong>${this._escape(prediction.predictions?.percent?.draw || "â€”")}</strong></div><div><span>Away chance</span><strong>${this._escape(prediction.predictions?.percent?.away || "â€”")}</strong></div></div></article>
  ...
  _cupsPage() {
    ...
    if (activeCup && !cupDataReady) {
      cupContent = `<article class="page-card"><div class="empty">Loading ${this._escape(activeCup.name)} dataâ€¦</div></article>`;
    ...
    return `
      ...
      ${activeCup ? `
        <section class="section cup-data">
          ...
          ${cupContent}
          ${false ? `
          ${!cupDataReady ? `<article class="page-card"><div class="empty">Loading ${this._escape(activeCup.name)} dataâ€¦</div></article>` : ""}
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="app-shell view-${this._viewMode}">
        ...
        <footer>Football Hub Â· Built for Home Assistant</footer>
      </div>
    `;
